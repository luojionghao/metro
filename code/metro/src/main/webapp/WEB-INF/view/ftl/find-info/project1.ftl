...
<input type="hidden" id="ppSvgUrl" value="${(pp.ppSvgUrl)!""}">
...
<input type="hidden" id="pingLorR" value="${(pingLorR)!""}">
<section class="content" style="background: white; padding:0px">
    ...
    <div class="main">
        ...
        <button id="leftbtn" type="button" style="width: 82px" class="btn btn-primary" onclick="clickLeft()">左  线</button>
        <button id="rightbtn" type="button" style="width: 82px" class="btn btn-primary" onclick="clickRight()">右  线</button>
  ...
  <!-- </div> -->
    <script>
        ...
        var settleArray = [];
        var ntervalSpListSize = '${ntervalSpList?size}';

        if(ntervalSpListSize != 0){
        <#list ntervalSpList as ntervalSp >
            settleArray.push({x: #{ntervalSp.mapX}, y: #{ntervalSp.mapY}, name: "${ntervalSp.spName!""}",intervalId:"${ntervalSp.intervalId!""}",SpId:"${ntervalSp.id!""}", date: "${ntervalSp.updateTime?datetime}"});
        </#list>
        ...
        function goNumL(goNum) {
            moveToNumL(goNum);
        }
        function goNumR(goNum) {
            moveToNumR(goNum);
        }
        ...
        function getLinedata() {
            ...
            var rLinekey = $("#rLinekey").val();
            if (rLinekey == null || rLinekey == "") {
                return;
            }
            $.ajax({
                type:"GET",
