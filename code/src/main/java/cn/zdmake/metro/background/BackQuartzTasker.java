...
public class BackQuartzTasker {
	...
	/*
	 * @Resource ICompanyAccountService companyAccountService;
	 * 
	 */
	/**
		...
		try {
			...
			for (IntervalWarningVO vo1 : list1) {
				...
				if (ir != null && ir.getCode() == 200) {
					...
					// 获取符合该环数要求的所有预警阈值信息
					List<IntervalWarningVO> list2 = lineIntervalWarningService.findQueryVOListForParams1(
							vo1.getIntervalId(), vo1.getLeftOrRight(), ringNum, vo1.getParam());
