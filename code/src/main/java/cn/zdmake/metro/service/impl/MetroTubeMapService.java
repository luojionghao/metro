...
public class MetroTubeMapService implements IMetroTubeMapService{
	...
	public TubeMapIntervalLr findLrInfo(Long userId, Long intervalId,Date loginTime) {
		...
		for(TubeMapInfo tm : lrs){
			...
			keys.add(key);
			String warn = tm.getWarningLevel()!=null?(tm.getWarningLevel()==1?"[红色上限预警]":(tm.getWarningLevel()==2?"[橙色上限预警]":(tm.getWarningLevel()==3?"橙色下限预警":"红色下限预警"))):null;
			...
			if(ir!=null){
				try{
					if("l".equals(tm.getLeftOrRight())){
						...
						lr.getLeft().setAdvanceStatus(StringUtil.nullToInt(ir.getResult() != null ? ir.getResult().get(key2).toString() : null));
					}else{
						if(ir.getResult() != null) {
							String result =	GsonUtils.toJson(ir.getResult().get(key1));
							ObjectMapper mapper = new ObjectMapper();
						...
						lr.getRight().setAdvanceStatus(StringUtil.nullToInt(ir.getResult() != null ? ir.getResult().get(key2).toString() : null));
					}
				}catch(Exception e){
